...
def format_telegram_output(data: Dict[str, Any]) -> str:
    ...
    lines = [f"🔍 *DKINT Search Results*", f"━━━━━━━━━━━━━━━━━━━━━", f"**Developer:** {config.DEVELOPER_TAG}", f"**Query:** `{data['query']}`", f"**Status:** ✅ {data['status'].upper()}", f"**Sources Found:** {data['sources_found']}", f"━━━━━━━━━━━━━━━━━━━━━"]
    ...
    for key, val in record.items():
        if key not in added and not key.startswith("_"):
            ...
            if isinstance(val, list):
                ...
                lines.append(f"**{label}:** {val}")
    lines.extend([f"━━━━━━━━━━━━━━━━━━━━━", f"📱 Generated by: @{config.BOT_USERNAME}"])
    return "\n".join(lines)

def save_json_file(data: Dict[str, Any], query: str) -> Optional[str]:
    try:
        temp_dir = Path(tempfile.gettempdir()) / "dkint_bot"
        temp_dir.mkdir(parents=True, exist_ok=True)
        filepath = temp_dir / f"dkint_{query}.json"
...
class AuthMiddleware(BaseMiddleware):
    async def __call__(self, handler, event: TelegramObject, data: Dict[str, Any]) -> Any:
        if isinstance(event, Message):
            ...
            if db.is_approved(msg.from_user.id):
                return await handler(event, data)
            ...
            cb = event
            if cb.data.startswith(("approve:","decline:")) or db.is_approved(cb.from_user.id):
                return await handler(event, data)
...
async def on_startup(bot: Bot):
    ...
    await bot.set_my_commands([{"command":"start","description":"Start the bot & register"},{"command":"num","description":"Search by phone number /num <number>"},{"command":"help","description":"Show help information"}])
...
async def cmd_num(message: Message, bot: Bot):
    user = message.from_user
    if message.chat.type in (ChatType.GROUP, ChatType.SUPERGROUP) and not db.is_approved(user.id):
...
async def cmd_approve_password(message: Message, command: CommandObject, bot: Bot):
    ...
    if config.ADMIN_IDS:
        try:
            await bot.send_message(config.ADMIN_IDS[0], f"🔑 *User Self-Approved via Password*\n\n**User:** {user.full_name}\n**Username:** @{user.username or 'N/A'}\n**User ID:** `{user.id}`", parse_mode="Markdown")
        except Exception:
            pass
